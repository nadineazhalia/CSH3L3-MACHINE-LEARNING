...
from sklearn.naive_bayes import GaussianNB # import NB
...
data_train = np.array(data_train).astype("float")
...
y_hasil = klasifikasi(data_train, 0.8, data_train)
print(y_hasil)
...
# fungi menghitung nilai akurasi
def akurasi(y, y_pred):
    count = 0
...
clf = svm.SVC()
clf.fit(X, y)
y_prediksi = clf.predict(X)
print("Akurasi SVM = " + str(akurasi(y,y_prediksi)))

print("Akurasi PNN = " + str(akurasi(y, y_hasil)))

# Naive Bayes
gnb = GaussianNB()
y_prediksi = gnb.fit(X, y).predict(X)
print("Akurasi NB = " + str(akurasi(y,y_prediksi)))

# MLP
clf = MLPClassifier()
clf.fit(X, y)
y_prediksi = clf.predict(X)
print("Akurasi MLP = " + str(akurasi(y,y_prediksi)))
